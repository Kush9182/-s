...
function gamestates(){
  ...
  if(gamestate==="level1"){
    ...
    if(detectColission(knight,spike1) || detectColission(knight,spike2) || detectColission(knight,spike3)){
  ...
  if(detectColissiona(knight,ground1)){
    Matter.Body.setVelocity(knight.body,{x:0,y:0})
  }
...
function detectColission(a,b){
  ...
  var distance=dist(bx,by,axy.x,axy.y);
  if(distance<=a.height/2+b.height/2 || distance<=a.width/2+b.width/2){
    return true
  }
  else{
    return false
...
function detectColissiona(a,b){
  axy=a.body.position
  bxy=b.body.position

  var distance=dist(bxy.x,bxy.y,axy.x,axy.y);
  ...
  if(distance<=a.height/3+b.height/2){
    return true
  }
  else{
    return false
